...
module.exports = (sender, msg, client, users, manager) => {
  ...
  const n = parseInt(msg.toUpperCase().replace('!BUYTF ', ''), 10);
  const amountofsets = n * rates.tf.sell;
  const maxKeys = parseInt(main.maxBuy / rates.tf.sell, 10);

  if (!Number.isNaN(n) && n > 0) {
    log.userChat(sender.getSteamID64(), language, `[ !BUYTF ${n} ]`);
    if (n <= maxKeys) {
      const theirKeys = [];
      const mySets = [];
                ...
                if (!ERR2) {
                  ...
                  if (amountofsets <= hisMaxSets) {
                    ...
                    chatMessage(
                      client,
                      sender,
                      messages.ERROR.OUTOFSTOCK.NOTUSED.TF[language].replace(
                        '{command}',
                        `!BUYANYTF ${n}`
                      )
                    );
    ...
    chatMessage(
      client,
      sender,
      messages.ERROR.INPUT.INVALID.TF[language].replace('{command}', '!BUYTF 1')
    );
  }
};
