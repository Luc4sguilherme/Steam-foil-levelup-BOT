      ...
      if (!ERR) {
        ...
        if (DATA) {
          ...
          if (hisMaxSets) {
            ...
            let message = ' ';
            if (cs > 0) {
              message += messages.CHECK.CURRENCIES.CSGO[language]
                .replace(/{CSGO}/g, cs)
                .replace('{SETS1}', cs * rates.csgo.sell);
            }
            if (hydra > 0) {
              message += messages.CHECK.CURRENCIES.HYDRA[language]
                .replace(/{HYDRA}/g, hydra)
                .replace('{SETS2}', hydra * rates.hydra.sell);
            }
            if (tf > 0) {
              message += messages.CHECK.CURRENCIES.TF[language]
                .replace(/{TF}/g, tf)
                .replace('{SETS3}', tf * rates.tf.sell);
            }
            if (parseInt(hisMaxSets, 10) > 0) {
              message += messages.CHECK.CURRENCIES.GEMS[language]
                .replace('{GEMS}', gems)
            ...
            if (CURRENTLEVEL >= 0) {
              ...
              if (CURRENTLEVEL !== i - 1) {
                chatMessage(
                  client,
                  sender,
                  messages.CHECK.RESPONSE[0][language]
                ...
                chatMessage(
                  client,
                  sender,
                  messages.CHECK.RESPONSE[1][language]
                    .replace('{SETS}', hisMaxSets)
                    .replace('{LEVEL}', i - 1)
                    .replace('{MESSAGE}', message)
                );
